...
// A plan is per-origin *and* per-device, so one starred on a laptop leaves the
// phone empty and there is no account to reconcile the two. The link below
// carries the plan itself rather than a reference to it — codes and pids in the
// fragment, which browsers never send to a server — so opening it on the phone
// is the whole of the transfer.
const planCode = () => `${[...plan].join(',')};${[...picks].join(',')}`;
const planLink = () =>
  `${location.origin}${location.pathname}#plan=${encodeURIComponent(planCode())}`;

// Merge, never replace: a phone with picks of its own should not lose them to a
// link sent from the laptop. Returns what the link actually added.
function readPlanLink() {
  const m = /[#&]plan=([^&]*)/.exec(location.hash);
  if (!m) return null;
  const added = { tracks: 0, talks: 0 };
  try {
    const [codes, pids] = decodeURIComponent(m[1]).split(';');
    (codes || '').split(',').filter(Boolean).forEach(c => {
      if (TRACK[c] && !plan.has(c)) { plan.add(c); added.tracks++; }
    });
    // pids cannot be checked here — talks.json may still be in flight — which is
    // also how a plan restored from storage is treated.
    (pids || '').split(',').filter(Boolean).forEach(p => {
      if (!picks.has(String(p))) { picks.add(String(p)); added.talks++; }
    });
  } catch (e) {
    console.warn('plan link not read:', e.message);
    return null;
  }
  persistPlan();
  // Drop it from the address bar: a reload should not re-apply a link that the
  // plan has since moved on from.
  try {
    history.replaceState(null, '', location.pathname + location.search);
  } catch (e) {
    console.warn('plan link not cleared from the address:', e.message);
  }
  return added;
}
...
// What a shared link brought in, reported once at the top of the tab so an
// unexpected jump in the counts has an explanation.
let arrived = null;
...
function renderPlan() {
  ...
  el.innerHTML = `
    ...
    ${arrived ? `<div class="note">${arrived.talks || arrived.tracks
      ? `Merged in from a link: ${[
          arrived.talks ? `${arrived.talks} talk${arrived.talks === 1 ? '' : 's'}` : '',
          arrived.tracks ? `${arrived.tracks} track${arrived.tracks === 1 ? '' : 's'}` : '',
        ].filter(Boolean).join(' and ')}.`
      : 'That link held nothing this device did not already have.'}</div>` : ''}
    <p class="lede">${empty
...
function renderPlanShare() {
  ...
  return `<h3 class="sec2 caps">Take it to your phone</h3>
    <p class="lede small">A plan is kept in the browser that made it, so a phone
      opens this site empty however many stars the laptop has. This link is the
      plan — send it to yourself and open it there, and everything above is
      merged into whatever that device already had. It goes in the part of an
      address that never leaves the browser, so nothing is uploaded.</p>
    ${local ? `<p class="note">This copy is open from disk, so the link points at
      this computer. Open the published site to get one a phone can follow.</p>` : ''}
    <div class="sharebar">
      <input class="search wide" id="planLink" readonly value="${esc(planLink())}"
        aria-label="Link carrying this plan">
      <button class="planbtn" id="planCopy">Copy link</button>
    </div>`;
}

function wirePlanShare() {
  const btn = $('#planCopy'), field = $('#planLink');
  if (!btn || !field) return;
  btn.onclick = async () => {
    field.select();
    try {
      ...
      // No clipboard API without a secure context; the old call still works,
      // and the text is selected either way for a manual copy.
...
function init() {
  ...
  loadPlan();
  // A link only ever adds to what this device had, so it is read after storage.
  arrived = readPlanLink();
  syncPlanUI();
  // Someone who followed a plan link came for the plan, not the programme.
  show(arrived ? 'plan' : 'schedule');
  loadTalks();
  loadPosters();
